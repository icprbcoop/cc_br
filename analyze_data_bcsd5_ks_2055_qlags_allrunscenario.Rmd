...
# Collect the scale factors in qscale0.df
...
# Use na.approx to fill in the NA's using linear interpolation
qscale70$qscale_cc70 <- zoo::na.approx(qscale70$qscale_cc70, qscale70$quantiles)

# Add qscale_cc70 to qscale0.df
qscale70 <- qscale70 %>%
  mutate(qscale_cc70 = round(qscale_cc70, 2)) %>%
  select(quantiles, qscale_cc70)
qscale0.df <- left_join(qscale0.df, qscale70, by = "quantiles")
...
# Use na.approx to fill in the NA's using linear interpolation
qscale30$qscale_cc30 <- zoo::na.approx(qscale30$qscale_cc30, qscale30$quantiles)

# Add qscale_cc30 to qscale0.df
qscale30 <- qscale30 %>%
  mutate(qscale_cc30 = round(qscale_cc30, 2)) %>%
  select(quantiles, qscale_cc30)

qscale0.df <- left_join(qscale0.df, qscale30, by = "quantiles")
...
qscale_display <- left_join(qm_cc50, qm_cc70, by = "quantiles")
qscale_display <- left_join(qscale_display, qm_cc90, by = "quantiles")
qscale_display <- left_join(qscale_display, qm_cc10, by = "quantiles")
qscale_display <- left_join(qscale_display, qm_cc30, by = "quantiles")
qscale_display <- left_join(qscale_display, qscaleall, by = "quantiles") %>%
  select(quantiles, qscale_cc90, qscale_cc70, qscale_cc50,
         qscale_cc30, qscale_cc10,  qscaleall_to2055,
...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# Write new PRRISM flow input time series file
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------

# Remember the scale factors are all in qscale0.df
#   and years in qscale0.df range from 1897 to 1979
#   whereas years in prrism_flows_orig.df range from 1929 to 2013
# Right now I can only figure out how to make PRRISM inputs for 1929 - 1979

# Get rid of all rows where year = NA
#   and we're using cc_all - forget about cc10 etc.
qscale.df <- qscale0.df %>%
  drop_na(year) %>% # 
  select(-qscale_cc50, -qscale_cc10, -qscale_cc30,
         -qscale_cc70, -qscale_cc90) %>%
  arrange(year)

prrism_flows_ccall <- left_join(prrism_flows_orig.df, 
                                qscale.df, by = "year") %>%
  filter(year < 1980) %>%
  mutate(jrr_inflow = round(jrr_inflow*qscaleall_to2055,2),
         sav_inflow = round(sav_inflow*qscaleall_to2055,2),
         sen_inflow = round(sen_inflow*qscaleall_to2055,2),
         occ_inflow = round(occ_inflow*qscaleall_to2055,2),
         pat_inflow = round(pat_inflow*qscaleall_to2055,2),
         por_no_nbr = round(por_no_nbr*qscaleall_to2055,2),
         por_below = round(por_below*qscaleall_to2055,2),
         lfalls_nat = round(lfalls_nat*qscaleall_to2055,2), # must be lfalls_no_res!
    ...
    select(-date_time, -quantiles, -q_mm, -q_obs_frac)


write_path <- "output/"
write_tsv(prrism_flows_ccall, paste(write_path, 
                                    "prrism_flows_ccall.txt", sep = ""),
          col_names = FALSE)
